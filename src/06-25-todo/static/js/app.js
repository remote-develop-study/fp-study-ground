...
(() => {
	...
	<li class="${todo.done && 'completed'}">
		<input class="toggle" type="checkbox" data-id="${todo.id}" ${todo.done && 'checked'}>
		<label>${todo.title}</label>
		<button data-id="${todo.id}" class="delete"/>
	</li>`

	const getCurrentId = () => state.todos.length !== 0 ? Math.max(...state.todos.map(todo => todo.id)) + 1 : 0
	...
	const addPropertyReactiveness = () => {
		Object.defineProperty(state, 'todos', {
			set(v) {
				state._todos = v
	...
	const bindDOMEvents = () => {
		...
		$('ul.filters li a')
			.map(DOM => DOM.addEventListener('click', e => {
		...
		$('.todo-list').addEventListener('click', e => {
			...
			if (e.target.classList.value === 'toggle') {
				state.todos = state.todos.map(todo => todo.id == e.target.dataset.id ? { ...todo,
					done: !todo.done
				} : todo)
			}
		})
